...
```{r chum join_tables, echo=FALSE, message=FALSE}
inner_join(chum_april_pre, chum_may_pre)|>
  kable()
```

The total number of sea lice per species and life stage in Table 17 of the PDF and using the extracted data are the same. However, there is a small discrepancy for the LEP Co and LEP C1 in May. To understand the discrepancy, I referred to the data included in Appendix III of the PDF (using the PDF rather that the extracted data):
...
So based on the results included in the Appendix III of the PDF, there were 0 LEP Co observed on chum salmon captured at pre-exposure locations in May.
...
So based on the results included in the Appendix III of the PDF, there were 2 LEP C1 observed on chum salmon captured at pre-exposure locations in May (as calculated using extracted data). In other words, the The results in Table 17 reproduced using extracted data are representative of the data in the Appendix III of the PDF. The discrepancy is explained. Successfully reproducing Table 17 validates the extracted data used to generate this table. Moving on to the next relevant table.
